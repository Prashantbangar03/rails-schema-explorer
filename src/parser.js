...
exports.parseStructureSql = parseStructureSql;
exports.parseSchemaRb = parseSchemaRb;
exports.parseSchemaDocument = parseSchemaDocument;
const TABLE_HEADER = /CREATE TABLE (?:(\w+)\.)?"?(\w+)"?\s*\(/g;
const PK_RE = /ALTER TABLE (?:ONLY )?(?:\w+\.)?(\w+)\s+ADD CONSTRAINT \w+ PRIMARY KEY \(([^)]+)\)/gi;
const INDEX_HEADER_RE = /CREATE (UNIQUE )?INDEX (\w+) ON (?:ONLY )?(?:\w+\.)?(\w+)\s+USING (\w+)\s*\(/gi;
const UNIQ_CONSTR_RE = /ALTER TABLE (?:ONLY )?(?:\w+\.)?(\w+)\s+ADD CONSTRAINT (\w+) UNIQUE \(([^)]+)\)/gi;
const CHECK_HEADER_RE = /ALTER TABLE (?:ONLY )?(?:\w+\.)?(\w+)\s+ADD CONSTRAINT (\w+) CHECK\s*\(/gi;
const EXCL_RE = /ALTER TABLE (?:ONLY )?(?:\w+\.)?(\w+)\s+ADD CONSTRAINT (\w+) EXCLUDE\s+([\s\S]+?);/gi;
const FK_RE = /ALTER TABLE (?:ONLY )?(?:\w+\.)?(\w+)\s+ADD CONSTRAINT (\w+) FOREIGN KEY \(([^)]+)\) REFERENCES (?:\w+\.)?(\w+)\(([^)]+)\)(?:\s+ON\s+DELETE\s+(\w+(?:\s+\w+)?))?/gi;
function balanceParens(sql, start) {
    let depth = 1;
    let i = start;
...
function pluralizeCandidates(base) {
    const out = new Set();
    out.add(base);
    out.add(base + 's');
...
function addInlineForeignKey(table, tables, fromCol, toTable, toCol, onDeleteRaw) {
...
function parseInlineTableConstraints(body, table, tables) {
    for (const part of splitTopLevelComma(body)) {
        ...
        m = trimmed.match(/^CONSTRAINT\s+"?(\w+)"?\s+FOREIGN KEY\s*\(([^)]+)\)\s*REFERENCES\s+(?:(\w+)\.)?"?(\w+)"?\s*\(([^)]+)\)(?:\s+ON DELETE\s+(\w+(?:\s+\w+)?))?/i);
        if (m) {
            addInlineForeignKey(table, tables, parseColList(m[2])[0], m[4], parseColList(m[5])[0], m[6]);
            continue;
        }
        m = trimmed.match(/^FOREIGN KEY\s*\(([^)]+)\)\s*REFERENCES\s+(?:(\w+)\.)?"?(\w+)"?\s*\(([^)]+)\)(?:\s+ON DELETE\s+(\w+(?:\s+\w+)?))?/i);
        if (m) {
            addInlineForeignKey(table, tables, parseColList(m[1])[0], m[3], parseColList(m[4])[0], m[5]);
            continue;
        }
...
function parseSqlColumnType(rest) {
...
function singularTableToken(word) {
...
function tableTokenMatchesParent(segment, parentTable) {
...
function tableNameJoinsParents(tableName, parentA, parentB) {
    ...
    for (let i = 1; i < parts.length; i++) {
        ...
        if (tableTokenMatchesParent(left, parentA) &&
            tableTokenMatchesParent(right, parentB)) {
            return true;
        }
        if (tableTokenMatchesParent(left, parentB) &&
...
function isJoinTableCandidate(t) {
    ...
    if (content.length <= JOIN_PIVOT_COL_MAX &&
...
function parseQuotedEnumValues(body) {
    return splitTopLevelComma(body)
        .map((part) => part.trim().replace(/^'|'/g, '').replace(/^"|"$/g, ''))
        .filter(Boolean);
}
function addCustomType(types, typeOrder, type) {
...
function parseSqlTypesAndViews(sql, types, typeOrder, views, viewOrder) {
    ...
    const viewRe = /CREATE (?:OR REPLACE )?(MATERIALIZED )?VIEW (?:(\w+)\.)?"?(\w+)"?\s+AS\s+/gi;
...
function parseRubyEnumTypes(ruby, types, typeOrder) {
    for (const m of ruby.matchAll(/^\s*create_enum\s+:?["']?(\w+)["']?\s*,\s*\[([^\]]+)\]/gm)) {
        const name = m[1];
...
function parseStructureSql(sql) {
    ...
    for (const m of sql.matchAll(INDEX_HEADER_RE)) {
        ...
        const colNames = parts.map(colNameFromExpr).filter((c) => Boolean(c));
...
function stripRubyQuotes(value) {
...
function parseRubyHashOptions(rest) {
    const opts = {};
    for (const m of rest.matchAll(/(\w+):\s*("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|:[\w]+|-?\d+(?:\.\d+)?|true|false|nil)/g)) {
        opts[m[1]] = stripRubyQuotes(m[2]);
    }
    return opts;
}
function formatRubyLiteral(raw) {
    if (raw === 'nil') {
        return 'NULL';
    }
...
function parseRubyColumnOptions(rest) {
...
function splitTopLevelCommaOutsideParens(s) {
    const out = [];
    let parenDepth = 0;
    let bracketDepth = 0;
    let cur = '';
...
function parseIndexFirstArg(firstArg) {
...
function defaultRubyIndexName(tableName, columnNames) {
    if (columnNames.length) {
        return `index_${tableName}_on_${columnNames.join('_and_')}`;
    }
    return `${tableName}_index`;
}
function parseRubyCheckConstraintArgs(args) {
...
function parseRubyAddCheckConstraint(rest) {
...
function parseRubyIndexArgs(args) {
    const parts = splitTopLevelCommaOutsideParens(args);
    if (parts.length === 0) {
        return null;
    }
...
function applyRubyIndex(table, ix) {
...
function inferFkColumnFromTable(toTable) {
...
function parseRubyOnDelete(value) {
    if (!value) {
        return null;
    }
...
function applyRubyForeignKeys(ruby, tables) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const ix of tableIndexes) {
            applyRubyIndex(table, {
                ...ix,
                name: ix.name ??
                    defaultRubyIndexName(tableName, ix.columnNames.length ? ix.columnNames : ix.columns),
...
function parseSchemaDocument(doc) {
